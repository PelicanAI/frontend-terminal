...
export default function EarningsTab() {
  ...
  const watchlistTickers = useMemo(
    () => new Set((watchlistItems ?? []).map((item: { ticker: string }) => item.ticker.toUpperCase())),
    [watchlistItems]
  )
...
What are the key things to watch? Any whisper numbers or sentiment shifts? How has this stock reacted to the last few earnings? What's the implied move from options?`
  ...
  const renderDayColumn = (day: typeof weekDays[0], i: number) => {
    ...
    const bmo = sortByImportance(dayEvents.filter((event) => event.hour === "bmo"))
    const amc = sortByImportance(dayEvents.filter((event) => event.hour === "amc" || event.hour === "dmh" || !event.hour))
    ...
    return (
      <m.div
        ...
        className={cn(
          "flex min-h-[400px] flex-col bg-[var(--bg-surface)]",
          "w-[200px] flex-shrink-0 md:w-auto"
        )}
      >
        <div
          className={cn(
            "border-b border-[var(--border-subtle)] p-3 text-center",
            isToday && "bg-[var(--accent-muted)]"
          )}
        >
          {isToday && <div className="-mt-3 mb-2 mx-auto h-0.5 w-8 rounded-full bg-[var(--accent-primary)]" />}
          <div className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-muted)]">{day.dayName}</div>
          <div className={cn(
            "font-[var(--font-geist-mono)] text-sm font-bold tabular-nums",
            isToday ? "text-[var(--accent-primary)]" : "text-[var(--text-primary)]"
          )}>
            {day.monthDay}
          </div>
          <div className="font-[var(--font-geist-mono)] text-[10px] tabular-nums text-[var(--text-muted)]">({dayEvents.length})</div>
        </div>
        ...
        {bmo.length > 0 && amc.length > 0 && <div className="mx-2 my-2 border-t border-[var(--border-subtle)]" />}
  ...
  return (
    <div>
      ...
      {!isLoading && events.length > 0 && <EarningsSpotlight events={events} onClick={handleClick} />}
      ...
      {isLoading ? (
        ...
        <m.div className="relative" initial="hidden" animate="visible" variants={staggerContainer}>
          <m.div
            className="hidden grid-cols-5 gap-px overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--border-subtle)] md:grid"
          ...
          <div className="scrollbar-hide overflow-x-auto md:hidden">
            <m.div
              className="flex min-w-[1000px] gap-px overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--border-subtle)]"
              variants={staggerContainer}
              initial="hidden"
              animate="visible"
            >
          ...
          <Image
            ...
            width={500}
            height={500}
            className="pointer-events-none absolute left-1/2 top-1/2 hidden h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/2 select-none object-contain opacity-[0.03] md:block"
      ...
      <div className="mt-2 py-3 text-center">
        <a
          ...
          className="text-[10px] text-[var(--text-disabled)] transition-colors duration-150 hover:text-[var(--text-muted)]"
        >
